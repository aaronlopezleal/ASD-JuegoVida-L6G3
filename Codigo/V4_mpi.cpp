...
void inicializarAleatorioGlobal(std::vector<int>& grid) {
    ...
    // Igual que en V1/V2/V3: solo interior, bordes globales a 0
    for (int i = 1; i < N - 1; ++i) {
        for (int j = 1; j < N - 1; ++j) {
...
void intercambiarFilasFantasma(std::vector<int>& gridLocal, int filasLocales, int rank, int size) {
    if (rank > 0) {
        ...
        std::fill(gridLocal.begin() + idxLocal(0, 0),
            gridLocal.begin() + idxLocal(1, 0), 0);
    }
    ...
    if (rank < size - 1) {
        ...
        std::fill(gridLocal.begin() + idxLocal(filasLocales + 1, 0),
            gridLocal.begin() + idxLocal(filasLocales + 2, 0), 0);
    }
}

void calcularSiguienteGeneracionLocal(
    ...
    // En principio ponemos todo el bloque siguiente a 0.
    // Así los bordes globales quedan correctamente muertos.
    std::fill(siguiente.begin(), siguiente.end(), 0);

    int primeraFila = 1;
    int ultimaFila = filasLocales;

    // Si soy el primer proceso, mi primera fila real corresponde a la fila global 0,
    // que es borde y no debe calcularse.
    if (rank == 0) {
        primeraFila = 2;
    }

    // Si soy el último proceso, mi última fila real corresponde a la fila global N-1,
    // que es borde y no debe calcularse.
    if (rank == size - 1) {
...
double ejecutarSimulacionMPI(int rank, int size, int& vivasTotales) {
    ...
    for (int g = 0; g < GENERACIONES; ++g) {
        ...
        calcularSiguienteGeneracionLocal(gridLocal, nextLocal, filasLocales, rank, size);
...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        std::cout << "V4 - MPI por division de filas\n";
        std::cout << "N = " << N << "\n";
    ...
    for (int r = 0; r < REPETICIONES; ++r) {
        int vivasTotales = 0;
        double tiempo = ejecutarSimulacionMPI(rank, size, vivasTotales);
    ...
    if (rank == 0) {
        std::cout << "\nMejor tiempo V4 MPI: " << mejorTiempo << " s\n";
        std::cout << "Celulas vivas finales V4: " << vivasMejor << "\n";
    }
